...
public class TimeLapseBuilder {
    ...
    public func build(with assetPaths: [String], atFrameRate framesPerSecond: Int32, type: AVFileType, toOutputPath: String) {
        DispatchQueue.global().async {
            ...
            if
                let firstAssetPath = assetPaths.last {
                let firstAssetURL = URL(fileURLWithPath: firstAssetPath)
                if let canvasSize = self.dimensionsOfImage(url: firstAssetURL) {
                    ...
                    do {
                        try self.videoWriter = AVAssetWriter(outputURL: videoOutputURL, fileType: type)
                            ...
                            videoWriterInput.requestMediaDataWhenReady(on: media_queue) {
                                ...
                                while !remainingAssetPaths.isEmpty {
                                    while videoWriterInput.isReadyForMoreMediaData {
                                        ...
                                        let nextAssetURL = URL(fileURLWithPath: nextAssetPath)
